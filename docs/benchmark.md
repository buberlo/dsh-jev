...
Generated by `node scripts/bench-video.mjs` from the recorded runs: the left
pane replays a baseline run that deleted the audit trail, the right pane
...
order, the outcomes come from the artifacts.

## Figure: value and cost at a glance

![Value and cost of live Jev](assets/bench-value.png)
